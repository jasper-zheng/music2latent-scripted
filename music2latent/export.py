...
class ScriptedResBlock(nn.Module):
    ...
    # def forward(self, x_in, time_emb=None):
    ...
    def __init__(self, hparams, sigma_rescale):
        ...
        for i, (num_layers,multiplier) in enumerate(zip(hparams.layers_list,hparams.multipliers_list)):
            ...
            for num in range(num_layers):
                ...
                down_layers.append(ScriptedResBlock(output_channels, output_channels, hparams, cond_channels = hparams.cond_channels, normalize=hparams.normalization, attention=hparams.attention_list[i]==1, heads=hparams.heads, use_2d=True))                
                input_channels = output_channels
            if i!=(len(hparams.layers_list)-1):
                output_channels = hparams.base_channels*hparams.multipliers_list[i+1]
                if hparams.freq_downsample_list[i]==1:
                    down_layers.append(ScriptedDownsampleFreqConv(input_channels, output_channels))
                else:
                    down_layers.append(ScriptedDownsampleConv(input_channels, output_channels, use_2d=True))
        ...
        for i, (num_layers,multiplier) in enumerate(zip(reversed(hparams.layers_list),multipliers_list_upsampling)):
            for num in range(num_layers):
                ...
                up_layers.append(ScriptedResBlock(input_channels, input_channels, hparams, cond_channels = hparams.cond_channels, normalize=hparams.normalization, attention=list(reversed(hparams.attention_list))[i]==1, heads=hparams.heads, use_2d=True))
            if i!=(len(hparams.layers_list)-1):
                output_channels = hparams.base_channels*multiplier
                if freq_upsample_list[i]==1:
                    up_layers.append(ScriptedUpsampleFreqConv(input_channels, output_channels))
    ...
    def forward_generator(self, latents, x):
        ...
        for i, layer in enumerate(self.down_layers):
            ...
            if l % self.layers_list[k] == 1 and l != self.layers_list[k]*2+1:
                d = layer(pyramid_latents[k])
                x = (x+d)/1.4142135 # np.sqrt(2.)
        ...
        for i, layer in enumerate(self.up_layers):
            l = l + 1
            if l % self.layers_list[k] == 1 and l != self.layers_list[k]*2+1:
                d = layer(pyramid_latents[-k-1])
                x = (x+skip_list.pop()+d)/1.732051 # np.sqrt(3.)
            elif l % self.layers_list[k] == 0 and l != self.layers_list[k]*2+1:
                x = layer(x)
